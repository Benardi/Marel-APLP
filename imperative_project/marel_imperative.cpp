...
bool is_valid_coordante(struct Coordinate coordinate) {
  return !(coordinate.row < 0) && !(coordinate.row > 2) && !(coordinate.column < 0) && !(coordinate.column > 2);
}
...
bool check_diagonals(struct Piece pieces[3]) {
  for (int i = 0; i < 3; i++) {
    ...
    bool secondary_diagonal = coordinate.row == 2 && coordinate.column == 0 || coordinate.row == 0 && coordinate.column == 2 || coordinate.row == 1 && coordinate.column == 1;
...
bool is_free(int x, int y) {
  return (marel_board[x][y] == '_');
}

void swap_piece(int x, int y, int new_x, int new_y, Player& player) {
  marel_board[x][y] = '_';
...
void computer_move_pieces(Player& player) {
  for (int i = 0; i < 3; ++i) {
    for (int j = 0; j < 3; ++j) {

      if (marel_board[i][j] == player.pieces[0].shape) {
        if (i != 1 && j != 1 && is_free(1, 1)) {
          swap_piece(i, j, 1, 1, player);
          return;
        }

        if (is_position_valid(i + 1, j)) {
          if (is_free(i + 1, j)) {
            swap_piece(i, j, i + 1, j, player);
            return;
          }
        }
  ...
  while (!is_victory) {
    ...
    if (!is_victory) {
      ...
      if (!has_computer_player) {
        ...
        computer_move_pieces(player_two);
      }
